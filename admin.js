...
function switchAdminTab(tab) {
  ...
  document.getElementById('panel-content').hidden = isStats;
  document.getElementById('panel-stats').hidden = !isStats;
  document.getElementById('tab-btn-content').classList.toggle('active', !isStats);
  document.getElementById('tab-btn-stats').classList.toggle('active', isStats);
  if (isStats) loadHatidjaStats();
}

async function loadHatidjaStats() {
  const ipBody = document.getElementById('stats-ip-body');
  const recentBody = document.getElementById('stats-recent-body');
  ...
  try {
    ...
    document.getElementById('stats-total').textContent = data.totalViews ?? 0;
    document.getElementById('stats-unique').textContent = data.uniqueIps ?? 0;
    document.getElementById('stats-updated').textContent =
      'Обновлено: ' + formatStatsDate(new Date().toISOString());

    if (!data.byIp?.length) {
      ipBody.innerHTML = '<tr><td colspan="4" class="stats-empty">Пока нет просмотров</td></tr>';
    } else {
      ipBody.innerHTML = data.byIp.map((row) => `
        <tr>
          <td>${escapeStatsHtml(row.ip)}</td>
          <td>${row.count}</td>
          <td>${formatStatsDate(row.firstSeen)}</td>
          <td>${formatStatsDate(row.lastSeen)}</td>
        </tr>
      `).join('');
    }

    if (!data.recent?.length) {
      recentBody.innerHTML = '<tr><td colspan="3" class="stats-empty">Пока нет просмотров</td></tr>';
    } else {
      recentBody.innerHTML = data.recent.map((row) => `
        <tr>
          <td>${formatStatsDate(row.at)}</td>
          <td>${escapeStatsHtml(row.ip)}</td>
          <td title="${escapeStatsHtml(row.userAgent)}">${escapeStatsHtml(row.userAgent) || '—'}</td>
        </tr>
      `).join('');
    }
  } catch {
    document.getElementById('stats-total').textContent = '—';
    document.getElementById('stats-unique').textContent = '—';
    document.getElementById('stats-updated').textContent = 'Не удалось загрузить статистику';
    ipBody.innerHTML = '<tr><td colspan="4" class="stats-empty">Ошибка загрузки</td></tr>';
    recentBody.innerHTML = '<tr><td colspan="3" class="stats-empty">Ошибка загрузки</td></tr>';
  }
}
...
document.getElementById('stats-refresh-btn')?.addEventListener('click', loadHatidjaStats);
...
document.getElementById('stats-clear-btn')?.addEventListener('click', async () => {
  ...
  if (res.ok) await loadHatidjaStats();
});
